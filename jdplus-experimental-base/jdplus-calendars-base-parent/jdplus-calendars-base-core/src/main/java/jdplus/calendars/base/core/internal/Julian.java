...
public class Julian implements Date {
    ...
    public static final long EPOCH = Gregorian.toFixed(0, DateUtility.DECEMBER, 30);
    ...
    public static Julian fromFixed(long date) {
        long approx = DateUtility.quotient(4 * (date - EPOCH) + 1464, 1461);
        long year = approx <= 0 ? approx - 1 : approx;
        long priorDays = date - toFixed(year, DateUtility.JANUARY, 1);
        int correction = date < toFixed(year, DateUtility.MARCH, 1) ? 0 : (isLeapYear(year) ? 1 : 2);
    ...
    public static boolean isLeapYear(long jYear) {
        return DateUtility.mod(jYear, 4) == (jYear > 0 ? 0 : 3);
    }
    ...
    public static long[] easternOrthodoxChristmas(long gYear) {
        return inGregorian(DateUtility.DECEMBER, 25, gYear);
    }
